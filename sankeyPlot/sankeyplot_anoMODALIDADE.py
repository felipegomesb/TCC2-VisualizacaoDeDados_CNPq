...
#BOTAO -> FILTRAR POR CATEGORIA

# build traces: global + one per categoria
fig = go.Figure()

# global trace (all categorias)
fig.add_trace(go.Sankey(
...
categorias = df_grouped['categoria'].drop_duplicates().tolist()

for categoria in categorias:
    df_sub = df_grouped[df_grouped['categoria'] == categoria]
...
# dropdown buttons: All + one per categoria
buttons = []
visible_all = [True] + [False] * len(categorias)
buttons.append(dict(label='Todas as categorias', method='update', args=[{'visible': visible_all}, {'title': 'Grande Área → Categoria → Ano'}]))

for i, categoria in enumerate(categorias):
    vis = [False] * (1 + len(categorias))
    vis[i + 1] = True
    buttons.append(dict(label=categoria, method='update', args=[{'visible': vis}, {'title': f'Categoria: {categoria} → Grande Área → Ano'}]))
...
fig.write_image('resultados/sankey/sankeyplot_button_categoria.svg', width=1200, height=800, scale=2)
